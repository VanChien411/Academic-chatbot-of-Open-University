  ...
  const renderUsers = () =>
    currentUsers.map((user: userAndChat, index: number) => (
      <tr key={user.user_id} onClick={() => handleListUser(user)}>
        ...
        <td>
          <div
            className="btn btn-success d-flex position-relative "
            style={{ width: "45px" }}
          >
            <svg
            ...
            {user.announcement != 0 && (
              <span
                className="d-flex position-absolute rounded-5  text-bg-danger"
                style={{
                  top: "-10px",
                  right: "-10px",
                  paddingLeft: "5px",
  ...
  return (
    <>
      ...
      <div className=" ">
        <Table
          hover
          className=" border border-primary rounded-2 shadow-sm   bg-light"
        ...
        >
          ...
          <tbody className="opacity-75">{renderUsers()}</tbody>
